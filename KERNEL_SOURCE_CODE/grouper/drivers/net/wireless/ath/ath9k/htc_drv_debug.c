				      ...
				      size_t count, loff_t *ppos)
{
	...
	len = snprintf(buf+len,sizeof(buf) - len , "%u %u %u\n" ,be32_to_cpu(cmd_rsp.xretries) ,be32_to_cpu(cmd_rsp.shortretries), be32_to_cpu(cmd_rsp.retransmission));
	// len += snprintf(buf + len, sizeof(buf) - len,
	// 		"%10s : %10u\n", "Xretries",
 ...
 //    len += snprintf(buf + len, sizeof(buf) - len,
	// 		"%10s : %10u\n", "TOTALRET",
	// 		be32_to_cpu(cmd_rsp.retransmission));
			       ...
			       size_t count, loff_t *ppos)
{
	...
	spin_lock_bh(&priv->tx.tx_lock);
	len +=snprintf(buf + len, sizeof(buf) - len, "%u %u %u %u %u %u %u %u\n", skb_queue_len(&priv->tx.mgmt_ep_queue),skb_queue_len(&priv->tx.cab_ep_queue),
		skb_queue_len(&priv->tx.data_be_queue),skb_queue_len(&priv->tx.data_bk_queue), skb_queue_len(&priv->tx.data_vi_queue),skb_queue_len(&priv->tx.data_vo_queue), 
		skb_queue_len(&priv->tx.tx_failed), priv->tx.queued_cnt);
	// len += snprintf(buf + len, sizeof(buf) - len, "%20s : %10u\n",
